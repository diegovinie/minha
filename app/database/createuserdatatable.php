...
function createUserdataTable(/*string*/ $prefix=null){
    ...
    $exe = $db->exec(
        "CREATE TABLE {$prx}userdata (
          ...
          `udata_nac` DATE DEFAULT NULL COMMENT 'Fecha de nacimiento',
          `udata_apt_fk` int(8) unsigned NOT NULL COMMENT 'numero de apartamento',
          `udata_user_fk` int(8) unsigned NOT NULL,

          PRIMARY KEY (`udata_id`),
          KEY `{$prx}udata_user` (`udata_user_fk`),
          KEY `{$prx}udata_apt` (`udata_apt_fk`),
          CONSTRAINT `{$prx}link_udata_user` FOREIGN KEY (`udata_user_fk`) REFERENCES `{$prx}users` (`user_id`),
          CONSTRAINT `{$prx}link_udata_apt` FOREIGN KEY (`udata_apt_fk`) REFERENCES `{$prx}apartments` (`apt_id`)
